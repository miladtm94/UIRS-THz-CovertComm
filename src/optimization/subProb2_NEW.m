...
Akn = alpha.*BW./log(2)./((PF_r+ PF_j)/(Po+Pi)/2); 
Bkn = (abs(H_ark).^2).*repmat(pa,numUsr,1)./(0.5*repmat(pj,numUsr,1).*abs(H_jk).^2 + 1); 
 

Ch = abs(H_ark).^2.*repmat(pa, numUsr, 1)./repmat(pj, numUsr,1)./(abs(H_jk).^2);
Cmkn =  zeros(numUsr-1,numUsr, N); % (K-1)-by-N
...
Phi = IRSbeamformingNEW_optim(alpha,Akn, Bkn, Cmkn, Ek, Ea, epsilon,idx);
